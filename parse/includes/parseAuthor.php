...
//	echo "Entry Str: $str<br />";
	...
	$str = preg_replace('/[\s,]+(and|&)[\s,]+/i', ', ', $str);			    	// Remove "&" / "and" with spaces or commas around.
	$str = preg_replace('@[(]|[)]|[/]|[:]|[\[]|[\]]|[<]|[>]@',' ', $str); 		// Remove unusual characters 
	
	$orig_cleaned_str = $str;  							// Original string that has been cleaned but not parsed.
...
//	$genericPattern[0] = '/^(\p{Lu}\p{M}*)(\p{L}\p{M}*)+[^,]*[,][ ]*(\p{Lu}\p{M}*)[^,]*[,]/';				// Generic form "Name, X. X.,"
//	$genericPattern[1] = '/^(\p{Lu}\p{M}*)[.][^,]*[ ](\p{Lu}\p{M}*)(\p{L}\p{M}*)[^,]*[,]/';					// Generic form "X. X. Name,"
//	$genericPattern[2] = '/^(\p{Lu}\p{M}*)(\p{L}\p{M}*)[^,]*[,][ ]*(\p{Lu}\p{M}*)[^,]*/';			    	// Generic form "Name, X. X."
//	$genericPattern[3] = '/^(\p{Lu}\p{M}*)[.][^,]*[ ](\p{Lu}\p{M}*)(\p{L}\p{M}*)[^,]*/';			    	// Generic form "X. X. Name"
//	$genericPattern[4] = '/^(\p{Lu}\p{M}*)(\p{L}\p{M}*)[^,]*[,][ ]*(\p{Lu}\p{M}*)(\p{L}\p{M}*)[^,]*[,]/';	// Generic form "Name, Name,"
//	$genericPattern[5] = '/^(\p{Lu}\p{M}*)(\p{L}\p{M}*)[^,]*[,][ ]*(\p{Lu}\p{M}*)(\p{L}\p{M}*)+[.]/';		// Generic form "Name, Name."
	
	$genericPattern[0] = '/^(\p{Lu})(\p{L})+[^,]*[,][ ]*(\p{Lu})[^,]*[,]/u';		// Generic form "Name, X. X.,"
	$genericPattern[1] = '/^(\p{Lu})[.][^,]*[ ](\p{Lu})(\p{L})[^,]*[,]/u';			// Generic form "X. X. Name,"
	$genericPattern[2] = '/^(\p{Lu})(\p{L})[^,]*[,][ ]*(\p{Lu})[^,]*/u';			// Generic form "Name, X. X."
	$genericPattern[3] = '/^(\p{Lu})[.][^,]*[ ](\p{Lu})(\p{L})[^,]*/u';			    // Generic form "X. X. Name"
	$genericPattern[4] = '/^(\p{Lu})(\p{L})[^,]*[,][ ]*(\p{Lu})(\p{L})[^,]*[,]/u';	// Generic form "Name, Name,"
	$genericPattern[5] = '/^(\p{Lu})(\p{L})[^,]*[,][ ]*(\p{Lu})(\p{L})+[.]/u';		// Generic form "Name, Name."
	...
	while(!empty($str)){   // Loop until $value string is empty
		...
		if($loop_control > 50) {$str = preg_replace('/^./','',$str); continue;}
...
//			$lnamePattern[1] = '/^(\p{Lu}\p{M}*)(\p{L}\p{M}*)+[-](\p{Lu}\p{M}*)(\p{L}\p{M}*)+[,]/';		// Compound last name "Name-Name,"
//			$lnamePattern[2] = '/^(\p{Lu}\p{M}*)(\p{L}\p{M}*)+[ ](\p{Lu}\p{M}*)(\p{L}\p{M}*)+[,]/';		// Split name "Name Name,"
//			$lnamePattern[3] = '/^(\p{Lu}\p{M}*)(\p{L}\p{M}*)+(\p{Lu}\p{M}*)(\p{L}\p{M}*)+[,]/';		// Mc's &  Mac's
//			$lnamePattern[4] = '/^(\p{Lu}\p{M}*)(\p{L}\p{M}*)+[ ]((\p{Lu}\p{M}*)[.]?[ ]?)+[,]/';		// Missing comma after last name
			
			$lnamePattern[0] = '/^(\p{Lu})(\p{L})+[,]/u';							// Regular last name "Name,"
			$lnamePattern[1] = '/^(\p{Lu})(\p{L})+[-](\p{Lu})(\p{L})+[,]/u';		// Compound last name "Name-Name,"
			$lnamePattern[2] = '/^(\p{Lu})(\p{L})+[ ](\p{Lu})(\p{L})+[,]/u';		// Split name "Name Name,"
			$lnamePattern[3] = '/^(\p{Lu})(\p{L})+(\p{Lu})(\p{L})+[,]/u';			// Mc's &  Mac's
			$lnamePattern[4] = '/^(\p{Lu})(\p{L})+[ ]((\p{Lu})[.]?[ ]?)+[,]/u';		// Missing comma after last name
...
//			$noCapsPattern = '/^(((\p{Lu}\p{M}*)[.][ ]*)+)((\p{L}\p{M}*)+)/'; // Pattern for no caps "X. X. name"
//			$lastPattern = '/(\p{L}\p{M}*)+/';								// Pattern for no caps last name "name"	
			
			$altPattern = '/^(Jr[.]*|Sr[.]*|III[.]*)/u';				// Pattern for Jr, Sr, III
			$noCapsPattern = '/^(((\p{Lu})[.][ ]*)+)((\p{L})+)/u'; 		// Pattern for no caps "X. X. name"
			$lastPattern = '/(\p{L})+/u';								// Pattern for no caps last name "name"		
...
//	$initPattern = '/^(\p{Lu}\p{M}*)[^(\p{L}\p{M}*)]/';	// Any leading capital letter
//	$removePattern = '/^(\p{Lu}\p{M}*)[.]*[ ]*[,]*/';	// Full initial with period, spaces, and commas
//	$altPattern = '/(Jr[.]*|Sr[.]*|III[.]*)/';			// Check for Jr, Sr, and III
	
	$initPattern = '/^(\p{Lu})[^(\p{L})]/u';			// Any leading capital letter
	$removePattern = '/^(\p{Lu})[.]*[ ]*[,]*/u';		// Full initial with period, spaces, and commas
	...
	// Remove spaces before after $initials and $alt before return
	return trim($initials.$alt);
}
...
//  $initPattern = '/^(\p{Lu}\p{M}*)[.]/';						// Pattern for initial
//	$altPattern = '/(Jr[.]*|Sr[.]*|III[.]*)/';					// Pattern for Jr, Sr, III
	
	$initPattern = '/^(\p{Lu})[.]/u';							// Pattern for initial
	$altPattern = '/(Jr[.]*|Sr[.]*|III[.]*)/u';					// Pattern for Jr, Sr, III
	...
	// Remove comma, period, and space from last name
	$lastname = trim(preg_replace('/[,]|[.]/', '', $name));  	

	// Remove spaces before after $initials and $alt before return
	return $lastname.', '.trim($initials.$alt).', ';
}
?>
