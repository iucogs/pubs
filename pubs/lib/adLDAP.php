...
class adLDAP {
	...
	var $_account_suffix="@ads.iu.edu";
	var $_base_dn = "OU=Accounts,DC=ads,DC=iu,DC=edu"; 
	
	// An array of domain controllers. Specify multiple controllers if you 
	// would like the class to balance the LDAP queries amongst multiple servers
	var $_domain_controllers = array ("ads.iu.edu");
	...
	function adLDAP($options=array()){
		// Turn of notice error reporting.
	...
	function user_info($username,$fields=NULL){
		...
		if(!isset($entries[0]["memberof"]["count"])) $entries[0]["memberof"]["count"]=0;
...
*/
	...
	// Search User based on filters
	function search_user($filter, $fields=NULL){
		if ($filter==NULL){ return (false); }
		if (!$this->_bind){ return (false); }
		
		// Create the filter 
		//$filter="(&(&(objectClass=user) (sn=E*))(givenName=R*))";
