...
Page.viewOwners_response = function()
{
	...
	{	
		...
		if ((responseObj.owners != undefined) && (responseObj.owners != false))
		{
			...
			html += 'Showing collections belonging to: <select id="setOwner" onchange="Page.changeSetOwnerSelectMenu(this);">';
			Page.proxies = new Array();
			html += '<option value=""></option>';
			
			for (var i=0; i<owners.length; i++)
			{
				Page.proxies.push(owners[i].username);
				if (owners[i].username == 'sep')
				{
				...
				else
				{
					...
					html += '>' + owners[i].lastname + ', ' + owners[i].firstname + '</option>';
				}
			}
			html += '</select>';
			if (Page.owner == "")
...
{
	//alert(Page.owner + ' ' + Page.proxies[0] + ' ' + Page.proxies[1] + ' ' + Page.proxies[2]);
	Page.hasProxy = Page.inArray(Page.owner, Page.proxies);	
//	alert(Page.hasProxy);
}
...
Page.addOrRemoveSelectMenu = function()
{
	//if(Page.inArray(Page.owner, Page.proxies)) // Owner is a proxy
	if(Page.hasProxy) // Owner is a proxy
	{
		Page.removeSelectMenuOption();  // Don't need extra empty option
	}
	else // Owner is not a proxy
	{
		Page.addSelectMenuOption();
	}
}

Page.addSelectMenuOption = function()
{
	if (document.getElementById('setOwner'))
	{
		var myselect=document.getElementById('setOwner');
		if(myselect.options[0].value != '') {
			try{
				myselect.add(new Option("", ""), myselect.options[0]) //add new option to beginning of "sample"
			}
			catch(e){ //in IE, try the below version instead of add()
				myselect.add(new Option("", ""), 0) //add new option to beginning of "sample"
			}
		}
		myselect.selectedIndex = 0; // Set empty option as index
	}	
}

Page.removeSelectMenuOption = function()
{
	if (document.getElementById('setOwner'))
	{
		var myselect=document.getElementById('setOwner');
...
/*	if (document.getElementById('setOwner'))
	{
		var myselect=document.getElementById('setOwner');
		if (!Page.hasProxy && (myselect.options[0].value != ''))
		{
			try{
			 myselect.add(new Option("", ""), myselect.options[0]) //add new option to beginning of "sample"
			}
	...
	var jsonStr = '{"request": {"type": "getCitationsGivenCollectionID",  "page": "' + Page.current_page + '", "citation_id_page": "' + citation_id + '", "citations_per_page": "' + Page.citations_per_page + '", "sort_order": "' + Page.sort_order + '", "collection_id": "' + Page.currentCollection + '", "citations": {"submitter": "' + Page.submitter + '", "owner": "' + Page.owner + '", "entryTime": ""}}}';
		//alert("Page.getCitationsGivenCollectionID: " + jsonStr);
