...
Page.showCheckAuthorPanel = function(responseObj)
{
	...
	else
	{
		...
		html += '<input type="button" value="Add Checked Authors Only" onclick="Page.checkInputAndSave(\'create_authors\',\''+ merge +'\',0);" />&nbsp;&nbsp;';
		html += '<input type="button" value="Do Not Add Authors" onclick="Page.uncheckAllAuthorCheckboxesInPanel();Page.checkInputAndSave(\'create_authors\',\''+ merge +'\',0);" />';
	}
...
Page.uncheckAllAuthorCheckboxesInPanel = function()
{
	for(var i = 0; i < 6; i++)
	{
		...
		{						
			document.getElementById('author'+i+'ln_checkbox').checked = "";
		}
	}
}
...
Page.inputMethod = function(input_method)
{
	...
	if ((input_method == 6) || (input_method == 7) || (input_method == 8) || (input_method == 16))
	{
...
Page.printBackTD = function(pointer_style)
{
	...
	if (((Page.state == 2) && (Page.current_newly_added_num > 0)) || ((Page.state == 3) && (Page.current_row_num > 0))) {
		...
		html += '<tr><td id="goToBack" align="left" width="10%" ' + pointer_style + ' onclick="Page.goToBackCitation();"><font size="+2">Back</font></td>';
	}
	else {
...
Page.printNextTD = function(pointer_style)
{
	...
	if (((Page.state == 2) && (Page.current_newly_added_num < Page.newly_added_citations.length-1)) || ((Page.state == 3) && (Page.current_row_num < Page._citations.length-1))) {
		html += '<td id="goToNext" align="right" width="10%" ' + pointer_style + ' onclick="Page.goToNextCitation();"><font size="+2">Next</font></td>';
	}
	else {
...
Page.update_page_dot_sentData = function(timestamp)
{
	...
	for(var i=0; i<6; i++) {
		...
		if (document.getElementById('author'+i+'ln_checkbox'))
		{
			if (document.getElementById('author'+i+'ln_checkbox').checked)
			{
				Page.sentData['author'+i+'id'] = -1; // php reads -1 as create new author
			}
			else
			{
				Page.sentData['author'+i+'id'] = -2; // php reads -2 as unverified author
			}
