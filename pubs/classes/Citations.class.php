...
class Citations
{
	...
	function save($args, $args_authors, $coll_id)
	{
		...
		$current_citation_id = $this->updateAuthorOf($args, $args_authors, $current_citation_id);
	...
	function updateAuthorOf($args, $args_authors, $current_citation_id)
	{
	...
	function checkAuthors($args, $args_authors, $coll_id)
	{
		...
		//	if (($lastname != "") && ($firstname != ""))
			...
			{
				list($author_id, $author_verified, $suggestions) = $this->determineWhetherAuthorIsVerified($lastname, $firstname);				
				if ($author_verified == 0) // author unverified (may or may not be in author table)
				{
					$author_id = -1;
					$problemFlag = 1;
				}
				else if(($author_id > 0) && ($author_verified == 1))// Valid author_id is returned and author is verified
				{
			...
			$authorsArray[$i][3] = array();//$suggestions;		
		}
	...
	function saveOrUpdateACitation($args, $args_authors, $coll_id,$verified)
	{
		$this->link = $this->connectDB();

		for ($i=0; $i<6; $i++)
		{
		if ($args_authors['author'.$i.'id'])
		{
			$temp_author_id = "";
			$verified_or_not = $args_authors['author'.$i.'id'];
			$lastname = trim($args_authors['author'.$i.'ln']);
			$firstname = trim($args_authors['author'.$i.'fn']);
			
			if ($verified_or_not == -2) // javascript says author should be unverified
			{
				list($author_id, $author_verified, $suggestions) = $this->determineWhetherAuthorIsVerified($lastname, $firstname);
			...
			else if ($verified_or_not >= -1)// javascript says author should be verified
			{
				list($author_id, $author_verified, $suggestions) = $this->determineWhetherAuthorIsVerified($lastname, $firstname);
				...
				{
					$temp_author_id = $this->verifyAuthor($author_id);
			...
			$args_authors['author'.$i.'id'] = $temp_author_id;
		}
		}
	...
	function determineWhetherAuthorIsVerified($lastname, $firstname)
	{
		...
		if (mysql_num_rows($result) > 0) 					// An author_id has been found
		{
			...
			$author_verified = $row['verified'];	
		}
		else // Not in author table
		{	
			$author_id = -1;  // -1 indicates no entry for the author in the author table
			$author_verified = 0;
			
			$query = "SELECT author_id, lastname, firstname FROM authors WHERE lastname='".mysql_real_escape_string($lastname)."'";
			$result1 = $this->doQuery($query, $this->link);
	...
	function verifyAuthor($author_id)
	{
		$this->link = $this->connectDB();
		
		$firstname = trim($author_id);
		$query = "SELECT * FROM authors WHERE author_id='".mysql_real_escape_string($author_id)."'";
		$result = $this->doQuery($query, $this->link);
		...
		else 
		{
			...
			$query = "UPDATE authors SET verified = 1 where author_id='".mysql_real_escape_string($author_id)."'";
			$result = $this->doQuery($query, $this->link);
	...
	function getCitations_byTimestamp_unverified($entryTime)
	{
		...
		function getCitations_byFac_all($submitter, $owner,$search,$type,$keyword,$sort_order, $citations_per_page, $page)
	{ 
...
// Ruth put code into this function to simplify
		...
		if (count($citations) > $citations_per_page)
		{
			...
			$citations = array_slice($citations, $start_num, $citations_per_page);
		}
	...
	function selectQueryFor_byFac_all($owner, $type, $keyword, $search,$sort_order)
	{
		$query = "SELECT * FROM $this->table WHERE submitter='".$owner."' ORDER BY citation_id ASC $this->limit";
	
		if($search=='search')
		{
			if($type=='title')
			{
		$query = "SELECT * FROM $this->table where title LIKE '%".$keyword."%'' ORDER BY citation_id ASC $this->limit"; 
			}
			if($type=='journal')
			{
        ...
        $query = "SELECT * FROM citations AS cit INNER JOIN author_of AS ao ON
        ao.citation_id = cit.citation_id INNER JOIN authors AS auths ON
        auths.author_id = ao.author_id WHERE auths.lastname LIKE
        '%".$keyword."%'";
			}
			if($type=='all')
			{
		$query = "SELECT * FROM $this->table where title='".$keyword."'OR journal='".$keyword."' OR author='".$keyword."' ORDER BY citation_id ASC $this->limit";
			...
			if($type=='citation_id')
			{
		$query = "SELECT * FROM $this->table where citation_id='".$keyword."' ORDER BY citation_id ASC $this->limit";
			}
		}
		...
		if($search=='all')
		{
			if($sort_order == 'year_desc')
			{
			$query = "SELECT * FROM $this->table where owner='".$owner."' ORDER BY year DESC $this->limit";
			}
			else
			{
	...
	function getCitationsGivenCollectionID($collection_id, $page, $citations_per_page, $submitter, $owner, $sort_order)
	{
		...
		$fullcollection_citations_array = $citations_array;
		
		$sections_array = $citations->getSections($collection_id);
		if ($sections_array)
			{
				$similar_citations_array = array();
		
				for ($i=1; $i<count($sections_array[1]); $i++)
				{
					$sections_array[1][i] = $this->sortCitations($citations_array[1][i], $sort_order);
				}
				$fullcollection_citations_array = $this->sortCitations($fullcollection_citations_array, $sort_order);
				return array($citations_array, $total_count, $similar_citations_array, 1, $fullcollection_citations_array,$sections_array);
			}
		else
		{
...
//		}
	...
	function sortCitations($citations, $sort_order)
	{
        ...
        $collator_master = new Collator('en_US');
	...
	function compare_by_author0ln($a, $b)
	{
		$collator = new Collator('en_US');
        $map = array('author0ln','author1ln','author2ln','author3ln','author4ln','author5ln','year','title');		
		foreach($map as $key)
		{	
			$retval = $collator->compare($a[$key], $b[$key]);
	...
	*/
	...
	function doFuzzyMatch($fuzzy_args, $citation_id)
	{
		...
		$query = "SELECT c.citation_id, a.lastname, c.year, c.title FROM citations c, authors a, author_of ao WHERE a.author_id = ao.author_id and c.citation_id = ao.citation_id AND ao.position_num = 1";
		
		// UNION SELECT c.citation_id, au.author0ln, c.year, c.title FROM citations c, authors_unverified au WHERE c.citation_id = au.citation_id";
	...
	function getSections($collection_id)
	{
				
		$this->link = $this->connectDB();

		$query = "SELECT DISTINCT section_title FROM section_of WHERE collection_id='".$collection_id."'";
		
		$result = $this->doQuery($query, $this->link);

		if(mysql_num_rows($result) > 0)
		{
			$section_title_arr = array();
			while($row = mysql_fetch_assoc($result))  // Copy result into an array.
			{
				$section_title_arr[] = $row['section_title'];
			}
			$citations_by_section_array = array();
			foreach($section_title_arr as $section_title)
			{
				$temp = array();
				$query = "SELECT citation_id FROM section_of WHERE section_title='".mysql_real_escape_string($section_title)."'";
				$result_citation_ids = $this->doQuery($query, $this->link);
				while($row = mysql_fetch_assoc($result_citation_ids))
				{				
					$temp[] = $row['citation_id'];
				}	
				$citations_by_section_array[] = $temp;
			}
			return array($section_title_arr, $citations_by_section_array);
		}
		else
		{
			return false;
